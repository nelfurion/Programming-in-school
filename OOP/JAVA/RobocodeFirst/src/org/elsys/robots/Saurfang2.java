...
public class Saurfang2 extends AdvancedRobot {
	...
	private boolean scannedForLast = false;
	...
	public void run() {
		...
		while (true) {
			if (this.scannedForLast && getRadarTurnRemaining() < 10) {
				scanSingleEnemy(enemy);
			}
	...
	public void scanSingleEnemy(EnemyBot enemy) {
		double turn = getHeading() - getRadarHeading() + enemy.getBearing();
		turn += 160 * radarDirection;
	...
	public void onScannedRobot(ScannedRobotEvent e) {
		...
		if (getRadarTurnRemaining() < 99999 - 360 * 2) {
			if (enemies.size() < 2 && !this.scannedForLast) {
				setTurnRadarRight(360);
				enemies = new HashMap<String, EnemyBot>();
				//scanSingleEnemy(enemy);
				this.scannedForLast = true;
			}
		}
	}
